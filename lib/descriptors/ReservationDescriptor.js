...
export default class ReservationDescriptor {
    ...
    constructor(descriptor, worker, ignoredProperties = []) {
        ...
        const requiredProperties = filter(ReservationProperties, p => ignoredProperties.indexOf(p) === -1);
        if (!every(requiredProperties, p => p in descriptor)) {
            const missing = difference(requiredProperties, Object.keys(descriptor)).join(', ');
