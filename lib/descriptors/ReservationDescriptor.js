 ...
 * @property {string} workerSid - The Sid of the {@link Worker} associated to this {@link Reservaiton}
...
export default class ReservationDescriptor {
    constructor(descriptor, config, ignoredProperties = []) {
        ...
        if (!_.isObject(config)) {
            throw new TypeError('Failed to instantiate ReservationDescriptor. <Configuration>config is required.');
        }
        ...
        if (!_.every(requiredProperties, p => p in descriptor)) {
            throw new TypeError('Failed to instantiate ReservationDescriptor. <Descriptor>descriptor does not contain all properties of a Reservation.');
        }
        ...
        const log = new Logger(`ReservationDescriptor-${this.sid}`, config._logLevel);

        // contains a task property, create a TaskDescriptor for it
        if (ignoredProperties.indexOf('task') === -1) {
            try {
                this.taskDescriptor = new TaskDescriptor(descriptor.task, config);
            } catch (err) {
