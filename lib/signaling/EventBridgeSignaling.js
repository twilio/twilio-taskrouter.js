...
import { SERVER_VERSION, twilioErrors as Errors, EB_URL_PARAMS } from '../util/Constants';
import { EventEmitter } from 'events';
import { Heartbeat } from '../util/Heartbeat';
import Logger from '../util/Logger';
import { verifyJWT } from '../util/Tools';
...
export default class EventBridgeSignaling extends EventEmitter {
    constructor(worker, options = {}) {
        ...
        // create log
        const log = new Logger(`EventBridgeSignaling-${worker.sid}`, worker._logLevel);
        const jwt = verifyJWT(worker._config.token);

        // start the clock on token expiration event
        const expiredJWTTime = jwt.exp;
        this.setTokenExpirationEvent(expiredJWTTime);
    ...
    updateToken(newToken) {
        ...
        const jwt = verifyJWT(newToken);
        this.setTokenExpirationEvent(jwt.exp);
        this._log.info('Updated token for Worker ' + jwt.grants.task_router.worker_sid);
    }

    setTokenExpirationEvent(expirationTime) {
        const timeToExpiration = expirationTime - new Date().getTime() - EXPIRATION_BUFFER;
        setTimeout(() => {
    ...
    createWebSocket() {
        const queryParam = `?${EB_URL_PARAMS.TOKEN}=${this.config.token}&${EB_URL_PARAMS.CLOSE_EXISTING_SESSIONS}=${this.closeExistingSessions}&${EB_URL_PARAMS.SERVER_VERSION}=${SERVER_VERSION}`;
        this.webSocket = new WS(this.config.WS_SERVER + queryParam);
        ...
        this.webSocket.onmessage = response => {
            this._log.debug('Received event');
