...
export default class EventBridgeSignaling extends EventEmitter {
    constructor(worker, options = {}) {
        ...
        // create log
        const log = new Logger(`EventBridgeSignaling-${worker.sid}`, 'debug');

        // TODO: this.setTokenExpirationEvent(expiredJWTTime);
    ...
    createWebSocket() {
        const queryParam = `?${EB_URL_PARAMS.TOKEN}=${this.config.token}&${EB_URL_PARAMS.CLOSE_EXISTING_SESSIONS}=${this.closeExistingSessions}&${EB_URL_PARAMS.CLIENT_VERSION}=${CLIENT_VERSION}`;
