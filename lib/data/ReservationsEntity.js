...
export default class ReservationsEntity extends EventEmitter {
    ...
    _cleanUpReservationAndTask(reservation) {
        this._log.debug('Soft deleting Reservation sid=%s for Worker %s', reservation.sid, this._config.workerSid);
        this._reservations.set(reservation.sid, [reservation, true]);

        setTimeout(() => {
            this._log.debug('Hard deleting of Reservation sid=%s for Worker %s', reservation.sid, this._config.workerSid);

            if (!this._reservations.delete(reservation.sid)) {
                this._log.warn('Failed to hard delete Reservation sid=%s for Worker %s', reservation.sid, this._config.workerSid);
            }
        }, 5000);
    }
}
