...
import { API_V1,
    RESERVATION_INSTRUCTION_ACCEPT,
    RESERVATION_INSTRUCTION_REJECT,
...
class Reservation extends EventEmitter {
    ...
    constructor(config, request, descriptor) {
        ...
        if (!(config instanceof Configuration)) {
            throw new TypeError('Failed to instantiate Reservation. <Configuration>config is a required parameter.');
        }
        ...
        this._config = config;
        this._log = new Logger(`Reservation-${descriptor.sid}`, config._logLevel);
        this._request = request;

        this.task = this._createTask(descriptor.taskDescriptor);
        Object.assign(this, descriptor);
        // taskDescriptor property should not be defined on the Reservation
    ...
    accept() {
        const requestURL = path.join(
            ...
            'Reservations', this.sid
        );
        const requestParams = { ReservationStatus: RESERVATION_INSTRUCTION_ACCEPT };
    ...
    reject(options = {}) {
        ...
        const requestParam = {
            ReservationStatus: RESERVATION_INSTRUCTION_REJECT,
        };
     ...
     * @param {TaskDescriptor} taskDescriptor - The {@link TaskDescriptor} that describes the {@link Task}
     * @private
     */
    _createTask(taskDescriptor) {
        if (!(taskDescriptor instanceof TaskDescriptor)) {
            this._log.error(`Error calling method _createTask() for Reservation sid=${this.sid}. <TaskDescriptor>taskDescriptor is a required parameter.`);
            return {};
        }

        try {
            const task = new Task(this._config, this._request, taskDescriptor);
            return task;
        } catch (err) {
    ...
    _update(latestReservationData, ignoredProperties = []) {
        try {
            const updatedReservationDescriptor = new ReservationDescriptor(latestReservationData, this._config, ignoredProperties);
            fieldsToUpdate.forEach(field => {
                ...
                if (field === 'task') {
                    ...
                    if (ignoredProperties.indexOf('task') === -1) {
                        ...
                        this.task._update(latestReservationData.task);
                    } else {
