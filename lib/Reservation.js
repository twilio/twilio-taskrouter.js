...
class Reservation extends EventEmitter {
    ...
    /**
     * Set whether the Conference related to this Reservation should end when the {@link Worker} drops from the call
     * @param {boolean} disconnect - Whether the Conference should end when this {@link Worker} participant leaves the Conference
     * @returns {Promise<this>} - Rejected if unable to set the Worker's leg in the Conference tied to the {@link Reservation}
     */
    setEndConferenceOnExit(disconnect) {
        if (!_.isBoolean(disconnect)) {
            throw new TypeError('Error calling method setEndConferenceOnExit(). <boolean>disconnect is a required parameter.');
        }

        const requestURL = this._worker.getRoutes().getRoute(WORKER_PARTICIPANT_INSTANCE, this.sid).path;

        const requestParams = {
            EndConferenceOnExit: disconnect,
    ...
    _createTask(reservationDescriptor) {
        ...
        try {
            const task = new Task(this._worker, this._request, reservationDescriptor.sid, taskDescriptor);
            return task;
        } catch (err) {
    ...
    _createTransfer(reservationDescriptor) {
        ...
        try {
            return new Transfer(this._worker, transferDescriptor);
        } catch (err) {
