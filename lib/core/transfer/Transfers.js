...
class Transfers extends EventEmitter {
     ...
     * @param eventType - the name of the transfer event in form { transfer-completed, transfer-initiated, ... }
     * @param rawEventData - the raw payload contents of the event
     * @private
     */
    _emitEvent(eventType, rawEventData) {
        ...
        if (this.outgoing) {
            ...
            if (this.outgoing.sid === rawEventData.sid) {
                // reset to the outgoing transfer to null on terminal transfer events since the transfer is no longer active
                if (Object.keys(_.pick(taskTransferEventTypes, ['transfer-completed', 'transfer-failed', 'transfer-canceled'])).indexOf(eventType) > -1) {
                    this._log.info('Terminal event transfer.%s was received for Worker %s. Attempting to reset transfers.active_outgoing_transfer \
                                    since there is no longer an active transfer.', eventName, this._worker.sid);
                    this._updateOutgoing(rawEventData);
                    this.outgoing._emitEvent(eventName, rawEventData);
                    // reset to null after the event with the latest this.outgoing has been emitted
                    this.outgoing = null;
                } else {
                    // only update and emit event for ['attemptFailed']
                    this._log.info('Non-terminal event transfer.%s was received for Worker %s. Proceeding to update the active outgoing transfer.', eventName, this._worker.sid);
                    this._updateOutgoing(rawEventData);
                    this.outgoing._emitEvent(eventName, this.outgoing);
                }
            } else {
                this._log.warn('The transfer specified by Event: transfer.%s does not match the current active outgoing transfer for Worker %s. Skipping event.', eventName, this._worker.sid);
            }
        } else {
            this._log.error('An active outgoing transfer does currently exist in Worker %s transfers map for Event: transfer.%s. Skipping event.', this._worker.sid, eventName);
        }
    }

    /**
     * Update the outgoing transfer object with the latest transfer data
     * @param latestTransferData The latest raw active transfer data.
