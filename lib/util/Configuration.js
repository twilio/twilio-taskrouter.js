...
export default class Configuration {
    ...
    constructor(token, options = {}) {
         ...
         * @type {boolean}
         */
        this.enableVersionCheck = options.enableVersionCheck;

        const regionSlice = options.region && options.region !== 'us1' ? `.${options.region}` : '';
        const eventBridgeHost = `event-bridge${regionSlice}.twilio.com`;

        this.GQL_WS_SERVER = `wss://${eventBridgeHost}/graphql-subscriptions`;

        if (options.ebServer || options.wsServer) {
            this._log.warn('"ebServer" and "wsServer" parameter will be removed in next major version. ' +
                'You may start using "region" instead.');
            return;
        }

        this.EB_SERVER = `https://${eventBridgeHost}/v1/wschannels`;
        this.WS_SERVER = `wss://${eventBridgeHost}/v1/wschannels`;
