...
import { reservationEventTypes, taskEventTypes, API_V1 } from './util/Constants';
...
import { validateOptions, verifyJWT } from './util/Tools';
...
class Worker extends EventEmitter {
    constructor(token, options = {}) {
        ...
        if (!_.isString(jwt.grants.task_router.role) && jwt.grants.task_router.role !== 'worker') {
            throw new TypeError('The TaskRouter Grant \'role\' must be specified as a \'worker\' in order to instantiate the Worker.');
        }

        if (!_.isString(jwt.grants.task_router.worker_sid)) {
            throw new TypeError('The TaskRouter Grant \'worker_sid\' must be supplied in order to instantiate the Worker.');
        }

        this.accountSid = jwt.sub;
        this.workspaceSid = jwt.grants.task_router.workspace_sid;
        this.sid = jwt.grants.task_router.worker_sid;

        this._log = new Logger(`Worker-${this.sid}`, this._logLevel);
        ...
        this._dataServices = {
            ...
            channelsEntity: new ChannelsEntity(this._config, this._request),
            reservationsEntity: new ReservationsEntity(this._config, this._request)
        };

        this._signaling = new EventBridgeSignaling(this, { closeExistingSessions: options.closeExistingSessions });
    ...
    _subscribeToSignalingEvents() {
        ...
        this._signaling.on('connected', () => {
            this._log.info('Received Event: \'connected\' from Signaling layer. Proceeding to initialize Worker %s.', this.sid);
            this._initialize();
        });
    ...
    _subscribeToTaskRouterEvents() {
        ...
        this._signaling.on('reservation.created', eventData => {
            ...
            if (eventData.sid) {
                try {
                    ...
                    throw new Error(`Failed to create a Resevation for Worker ${this.sid} on Event: reservationCreated. Error: ${err}`);
                }
            } else {
        ...
        this._signaling.on('reservation.accepted', eventData => {
            this._log.info('Worker %s received Event: reservation.accepted.', this.sid);
            if (eventData.sid) {
                const reservation = this.reservations.get(eventData.sid);
                if (reservation) {
                    reservation._emitEvent('accepted', eventData);
                } else {
                    this._log.error('The Resevation sid=%s specified in Event: reservation.accepted does not ' +
                                        'exist in the Worker\'s map of Reservations. Unable to update Worker %s reservation.', eventData.sid, this.sid);
                    throw new Error(`Failed to update Worker ${this.sid} reservation.`);
                }
            } else {
                this._log.error('Event: reservationAccepted did not contain a Reservation sid. Unable to update Reservation for Worker %s', this.sid);
        ...
        Object.keys(taskEventTypes).forEach(eventType => {
            this._signaling.on(`task.${eventType}`, eventData => {
                ...
                if (eventData.sid) {
                    this._dataServices.reservationsEntity._emitTaskEvent(eventType, eventData);
                } else {
                    this._log.error('Event: reservation.%s did not contain a Reservation sid. Unable to update Reservation for Worker %s.', eventType, this.sid);
                    throw new Error(`Failed to update Reservation for Worker ${this.sid}.`);
                }
    ...
    _initialize() {
        ...
        this._request.get(requestURL, API_V1).then(response => {
            ...
            this.activities = this._dataServices.activitiesEntity.activities;
            this._log.info('Worker %s activities successfully initialized', this.sid);

            this.channels = this._dataServices.channelsEntity.channels;
            this._log.info('Worker %s channels successfully initialized', this.sid);

            this.reservations = this._dataServices.reservationsEntity.reservations;
            this._log.info('Worker %s currently pending reservations successfully initialized', this.sid);
    ...
    _setWorkerConnectActivity() {
        if (!_.isString(this._connectActivitySid)) {
            throw new TypeError('Failed to set the Worker\'s activity to the provided optional connectActivitySid. <string>connectActivitySid is a required parameter.');
        }
        ...
        return this._updateWorkerActivity(this._connectActivitySid).then(() => {
            ...
            this._log.error('Unable to set Worker %s activity to the provided %s on successful connection. Error: %s', this.sid, this._connectActivitySid, err);
            throw err;
        });
    }
     ...
     * @private
     */
    _updateWorkerActivity(activitySid) {
