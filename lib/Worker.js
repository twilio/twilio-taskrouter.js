                ...
                if (eventData.initiating_reservation_sid && eventData.task_sid) {
                    ...
                    if (reservation) {
                        if (reservation.task.sid === eventData.task_sid) {
                            ...
                            reservation.task._emitOutgoingTransferEvent(eventType, eventData);
                        } else {
                            this._log.warn('The task %s related to Event: task.%s does not match the expected owning Reservation %s for Worker %s. Skipping event.', eventData.task_sid, reservation.sid, eventType, this.sid);
                        }
                    } else {
