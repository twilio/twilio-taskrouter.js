...
import { reservationEventTypes, taskEventTypes, taskTransferEventTypes, API_V1, WORKER_UPDATE_OPTIONS, CREATE_TASK_OPTIONS } from './util/Constants';
...
class Worker extends EventEmitter {
     ...
     * @param {string} taskQueueSid - The Sid of the {@link TaskQueue} this Task should belong to, used for reporting purposes only
     * @param {Worker.TaskOptions} [options]
     * @returns {Promise<String>} - Rejected if unable to create a Task on behalf of the {@link Worker}. Returns the TaskSid of the created Task.
     *//**
     * @typedef {Object} Worker.TaskOptions
     * @property {object} [attributes=null] - Additional attributes for the {@link Task}
     * @property {string} [taskChannelUniqueName=null] - The friendly name of the {@link Channel} this {@link Task} belongs to. If not provided, defaults to 'default' channel.
     * @property {string} [taskChannelSid=null] - The Sid of the {@link Channel} this {@link Task} belongs to. If not provided, defaults to 'default' channel.
     */
    ...
    _subscribeToTaskRouterEvents() {
        ...
        this._signaling.on('reservation.failed', eventData => {
            this._log.info('Worker %s received Event: reservation.failed.', this.sid);

            this.emit('reservationFailed', eventData);
        });
                ...
                if (eventData.initiating_reservation_sid && eventData.task_sid) {
                    ...
                    if (reservation) {
                        if (reservation.task.sid === eventData.task_sid) {
                            ...
                            reservation.task._emitEventForOutgoingTransfer(eventType, eventData);
                        } else {
                            this._log.warn('The task %s related to Event: task.%s does not match the expected owning Reservation %s for Worker %s. Skipping event.', eventData.task_sid, eventType, reservation.sid, this.sid);
                        }
                    } else {
                        this._log.warn('The reservation %s related to Event: task.%s does not exist in Worker %s Reservations map. Skipping event.', eventData.initiating_reservation_sid, eventType, this.sid);
                    }
                } else {
