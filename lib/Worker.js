...
import { reservationEventTypes, taskEventTypes, taskTransferEventTypes, API_V1 } from './util/Constants';
 ...
 * @property {string} [region] - the ingress region for connections (ex. "ie1")
 */
    ...
    _subscribeToTaskRouterEvents() {
        ...
        Object.keys(taskEventTypes).forEach(eventType => {
            this._signaling.on(`task.${eventType}`, eventData => {
                ...
                if (eventData.sid) {
                    const task = this._dataServices.reservationsEntity.getTask(eventData.sid);
                    task._update(eventData);
                    task._emitEvent(eventType, eventData);
                } else {
                    this._log.error('Event: reservation.%s did not contain a Task sid. Unable to emit event for Worker %s.', eventType, this.sid);
        ...
        Object.keys(taskTransferEventTypes).forEach(eventType => {
            this._signaling.on(`task.${eventType}`, eventData => {
                ...
                if (eventData.task_sid) {
                    const task = this._dataServices.reservationsEntity.getTask(eventData.task_sid);
                    eventType = eventType.replace(/-([a-z])/g, g => g[1].toUpperCase());
                    task._emitEvent(eventType, eventData);
                } else {
                    this._log.error('Event: reservation.%s did not contain a Task sid. Unable to emit event for Worker %s.', eventType, this.sid);
                    throw new Error(`Failed to emit event for Worker ${this.sid}.`);
