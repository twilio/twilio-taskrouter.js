...
export default class TaskRouterEventHandler {
    ...
    _workerCapacityUpdateHandler(eventData) {
        ...
        // We must increment worker's version here manually, because it is incremented in the backend
        // when capacity changes, but the eventData contains only channel data, so we don't receive
        // the updated worker data with this event.
    ...
    _workerChannelAvailabilityUpdateHandler(eventData) {
        ...
        // We must increment worker's version here manually, because it is incremented in the backend
        // when channel availability changes, but the eventData contains only channel data, so we don't receive
        // the updated worker data with this event.
        this._worker._bumpVersion();
    ...
    _reservationCreatedHandler(eventData) {
        ...
        if (eventData.sid) {
            try {
                ...
                // We must bump worker's version here, because it is incremented in the backend when a new reservation is added,
                // but there is no worker-specific event for it.
                this._worker._bumpVersion();
