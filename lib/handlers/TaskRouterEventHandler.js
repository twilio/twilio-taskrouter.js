...
import { taskEventTypes, taskTransferEventEmitterMapping } from '../util/Constants';
...
export default class TaskRouterEventHandler {
    ...
    _workerAttributesUpdateHandler(eventData) {
        ...
        this._worker('attributesUpdated', this._worker._update(eventData));
    }
    ...
    _reservationUpdateHandler(eventData, eventType) {
        this._log.info(`Worker %s received Event: ${eventType}.`, this._worker.sid);
        if (eventData.sid) {
            const reservation = this._worker.reservations.get(eventData.sid);
            if (reservation) {
                reservation._emitEvent(eventType, eventData);
            } else {
                this._log.error(`The Resevation sid=${eventData.sid} specified in Event: reservation.${event.eventType} does not \
            ...
            this._log.error(
                `Event: reservation.${event.eventType} did not contain a Reservation sid. Unable to update Reservation for Worker ${this._worker.sid}`);
    ...
    _reservationCleanupEventsHandler(eventData, eventType) {
        this._log.info('Worker %s received Event: %s.', this._worker.sid, eventType);
