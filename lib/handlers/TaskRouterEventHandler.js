...
import { taskEventTypes, taskTransferEventEmitterMapping, reservationToSDKEventsMapping, participantEventEmitterMapping } from '../util/Constants';
...
export default class TaskRouterEventHandler {
    ...
    getTREventsToHandlerMapping() {
        return {
            ...
            'participant.added': '_participantEventHandler',
            'participant.add.failed': '_participantEventHandler',
            'participant.removed': '_participantEventHandler',
            'participant.remove.failed': '_participantEventHandler',
            'participant.modified': '_participantEventHandler',
            'participant.modify.failed': '_participantEventHandler'
        };
    }
    ...
    _reservationCreatedHandler(eventData) {
        ...
        if (eventData.sid) {
            try {
                ...
                throw new Error(`Failed to create a Reservation for Worker ${this._worker.sid} on Event: reservationCreated. Error: ${err}`);
            }
        } else {
    ...
    _participantEventHandler(eventData, eventType) {
        const mappedEventType = participantEventEmitterMapping[eventType];
        this._log.info('Worker %s received Event: %s mapped to %s.', this._worker.sid, eventType, mappedEventType);
        const taskSid = eventData.routing_properties && eventData.routing_properties.task_sid;
        if (taskSid) {
            const tasks = this._worker._dataServices.reservationsEntity.getTasks(taskSid);
            if (tasks && tasks.length > 0) {
                tasks[0]._emitEvent(mappedEventType, eventData);
            } else {
                this._log.warn(
                    'The task specified by Event: participant.%s does not exist in Worker %s Reservations map. Skipping event.',
                    mappedEventType, this._worker.sid);
            }
        } else {
            this._log.error('Event: participant.%s did not contain a Task sid. Unable to emit event for Worker %s.',
                            mappedEventType, this._worker.sid);
            throw new Error(`Failed to emit event for Worker ${this._worker.sid}.`);
        }
    }
