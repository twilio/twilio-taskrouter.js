...
class Task extends EventEmitter {
     ...
     * @property {Object} [attributes] - Updated attributes for the task
     ...
     * @property {string} [holdUrl=default Twilio hold music] - The URL endpoint to play when participant is on hold.
     * @property {string} [holdMethod='GET'] - The HTTP method for the hold URL.
     */
     ...
     * @param {Task.HoldOptions} [options]
     *//**
     * @typedef {Object} Task.HoldOptions
     * @property {string} [holdUrl=default Twilio hold music] - The URL endpoint to play when participant is on hold.
     * @property {string} [holdMethod='GET'] - The HTTP method for the hold URL.
    ...
    async _emitEventForOutgoingTransfer(eventType, rawEventData) {
        ...
        let retryCount = 0;
        while (!this.transfers.outgoing && retryCount < 20) {
            await new Promise((resolve) => setTimeout(resolve, 500));
            retryCount++;
            this._log.debug('Transfer event arrived, waiting for API response, because transfer object missing', retryCount);
        }

        retryCount = 0;
        while (this.transfers.outgoing && this.transfers.outgoing.sid !== rawEventData.sid && retryCount < 20) {
            await new Promise((resolve) => setTimeout(resolve, 500));
            retryCount++;
            this._log.debug('Transfer event arrived, waiting for API response to get the correct transfer SID', retryCount);
        }
