...
class Task extends EventEmitter {
    ...
    constructor(worker, request, reservationSid, descriptor) {
         ...
         * @type {Date}
         */
        this.virtualStartTime = descriptor.virtualStartTime;
    ...
    /**
     * Set virtual start time of the {@link Task}.
     * @param {Date} date - Virtual start time as a Date.
     * @returns {Promise<this>} - Rejected if fails to set the time.
     */
    setVirtualStartTime(date) {
        if (!isDate(date)) {
            throw new TypeError('Unable to set virtual start time on Task. <Date>date is a required parameter.');
        }

        const requestURL = this._worker.getRoutes().getRoute(TASK_INSTANCE, this.sid).path;
        const requestParams = { VirtualStartTime: date.toISOString().slice(0, 19) + 'Z' };
    ...
    updateParticipant(options) {
        const types = {
            hold: (val) => isBoolean(val),
            holdUrl: (val) => isString(val),
            holdMethod: (val) => isString(val)
        };
    ...
    async _emitEventForOutgoingTransfer(eventType, rawEventData) {
        ...
        if (this.transfers.outgoing && this.transfers.outgoing.sid === rawEventData.sid) {
            if (Object.keys(pick(taskTransferEventTypes, [TRANSFER_INITIATED])).indexOf(eventType) > -1) {
