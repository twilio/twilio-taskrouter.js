...
class Task extends EventEmitter {
    ...
    _emitOutgoingTransferEvent(eventType, rawEventData) {
        this._log.trace(`_emitOutgoingTransferEvent(${eventType}, ${JSON.stringify(rawEventData)})`);

        if (!_.isString(eventType)) {
            throw new TypeError('Error calling _emitOutgoingTransferEvent(). <string>eventType is a required parameter.');
        }

        if (!_.isObject(rawEventData)) {
            throw new TypeError('Error calling method _emitOutgoingTransferEvent(). <object>payload is a required parameter.');
        }
        ...
        if (this.transfers.outgoing && this.transfers.outgoing.sid === rawEventData.sid) {
            if (eventType === 'transfer-initiated') {
                ...
                this.emit('transferInitiated', this.transfers.outgoing);
            } else {
